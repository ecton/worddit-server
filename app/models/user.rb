...
class User < CouchRest::ExtendedDocument
  ...
  def check_password(password)
    User.hash_password(password) == self.password_hash
  end
  
  def setup_auth_token(client_type, device_id)
    # lookup an existing auth or create a new one
    self.auths ||= []
    auth = self.auths.find_all{|a| a.client_type == client_type && a.device_id == device_id}.first
    if auth.nil?
      auth = UserAuthentication.new(:client_type => client_type, :device_id => device_id)
      self.auths << auth
    end
    
    auth.authentication_token = self.id + Digest::SHA1.hexdigest((0...8).map{65.+(rand(25)).chr}.join + client_type + device_id)
